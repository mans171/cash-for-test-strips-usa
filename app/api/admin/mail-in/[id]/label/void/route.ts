...
// "Void label". Admin only, session check first. Nothing is cleared: the
// shipment id, tracking code and label URLs stay on the row as history, and
// label_refund_status is what marks the label dead (lib/mail-in hasActiveLabel)
// — the seller page stops offering it and a new label may then be made.
//
...
export async function POST(request: Request, { params }: Context) {
  try {
    ...
    try {
      refundStatus = await refundShipment(config, order.easypost_shipment_id as string)
    } catch (error) {
    ...
    const saved = await supabaseAdmin
      .from('mail_in_orders')
      .update({ label_refund_status: refundStatus, status: 'quote_agreed', updated_at: nowIso })
      .eq('id', id)
      .eq('status', 'label_made')
      .select('*')
      .maybeSingle()
    if (saved.error || !saved.data) {
      console.error(`[POST label/void] REFUND REQUESTED BUT NOT SAVED — kit ${order.order_number}, shipment ${order.easypost_shipment_id}, refund ${refundStatus}`)
    ...
    const { error: eventError } = await supabaseAdmin.from('mail_in_events').insert([
      { order_id: id, type: 'label_voided', detail: { refund_status: refundStatus, tracking_code: order.tracking_code }, actor: 'admin' },
      { order_id: id, type: 'status_changed', detail: { from: 'label_made', to: 'quote_agreed' }, actor: 'admin' },
    ])
