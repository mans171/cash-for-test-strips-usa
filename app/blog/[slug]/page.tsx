...
export async function generateStaticParams() {
  return [
    ...STATE_BLOG_POSTS.map((post) => ({ slug: post.slug })),
    // Non-state posts from the registry — one TS file per post in lib/posts/.
    ...POST_REGISTRY.map((post) => ({ slug: post.slug })),
  ];
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // Registry posts take priority over "post not found" — check them first.
  const statePost = getPostBySlug(slug);
  if (!statePost) {
  ...
  const written = bodyFor(statePost.stateCode);
  const title = written?.title ?? postTitle(statePost.stateCode, statePost.stateName);
  const description =
    written?.metaDescription ?? postMetaDescription(statePost.stateCode, statePost.stateName);
...
export default async function BlogPostPage({ params }: Props) {
  ...
  // --- Registry post (non-state) fast path -----------------------------------
  // When the slug matches a registry post rather than a state post, render the
  // lightweight registry template and return early. This is the content path
  // for Wednesday topical posts — no Supabase lookup, no angle/health data.
  const statePost = getPostBySlug(slug);
  if (!statePost) {
    const registryPost = getRegistryPost(slug);
